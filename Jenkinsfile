            ...
            steps {
                ...
                script {
                    docker.withRegistry("${env.DOCKER_REGISTRY_URL}", 'docker_registry_credentials') {
                        def customImage = docker.build("notification-service")
            ...
            steps {
                ...
                withCredentials([
                    ...
                    dir("lpg-terraform-paas/environments/master") {
                        ...
                        sh "terraform plan -target=module.notification-service -var 'notification_service_docker_tag=${env.BRANCH_NAME}-${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}'"
                        sh "terraform apply -target=module.notification-service -var 'notification_service_docker_tag=${env.BRANCH_NAME}-${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}' -auto-approve"
                    }
                }
            }
        }
